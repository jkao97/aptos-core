...
use crate::{
    ...
    v2::data_cache::{TransactionDataCache, TransactionDataCacheAdapter},
    verifier::{event_validation, script_validation, transaction_arg_validation},
};
...
use move_vm_runtime::{
    ...
    AsFunctionValueExtension, LoadedFunction, ModuleStorage, RuntimeEnvironment,
    WithRuntimeEnvironment,
};
use move_vm_types::gas::{GasMeter, UnmeteredGasMeter};

pub trait AptosSession {
    type CodeView: ModuleStorage;
...
impl<'a, DataView, CodeView> AptosSession for Session<'a, DataView, CodeView>
where
...
impl<'a, DataView, CodeView> Session<'a, DataView, CodeView>
where
    DataView: AptosMoveResolver,
    CodeView: AptosModuleStorage,
{
    ...
    pub(crate) fn execute_unmetered_system_function_once(
        ...
        self.execute_unmetered_system_function(module_id, function_name, serialize_values(&args))
            .or_else(|err| {
                // TODO: revisit this remapping, maybe we can clean it up?
    ...
    pub(crate) fn materialize_writes(&mut self) -> Result<(), VMStatus> {
        ...
        let delayed_field_ids = aggregator_context
            .materialize(&self.new_slot_metadata)
        ...
        table_context
            .materialize(
                ...
                &self.code_view.as_function_value_extension(),
    ...
    pub(crate) fn build_gas_meter<F, G>(&self, make_gas_meter: F, initial_balance: Gas) -> G
    where
        G: AptosGasMeter,
...
pub struct UserTransactionSession<'a, DataView, CodeView> {
    /// Underlying session for executing Move code.
    pub(crate) session: Session<'a, DataView, CodeView>,
...
where
    ...
    CodeView: AptosModuleStorage,
{
    pub(crate) fn build_gas_meter<F, G>(&self, make_gas_meter: F) -> G
    where
        G: AptosGasMeter,
        F: FnOnce(u64, VMGasParameters, StorageGasParameters, bool, Gas, &'a CodeView) -> G,
    {
    ...
    CodeView: AptosCodeStorage,
{
    pub(crate) fn execute_user_transaction(
        ...
        // prologue terminates.
        // TODO: charge gas for the whole prologue?
    ...
    CodeView: AptosCodeStorage,
{
    fn execute_user_transaction_payload(
        ...
        if let Some(multisig_address) = self.txn.multisig_address() {
            ...
            if let Err(status) = self.execute_multisig_payload(gas_meter, &payload) {
                ...
                self.charge_gas_for_state_changes(gas_meter)?;
            ...
            match self.executable {
                ...
                // TODO: Add a test that this is checked.
    ...
    fn execute_script(
        ...
        let func = self
            .session
            .code_view
            .load_script(serialized_script.code(), serialized_script.ty_args())?;
    ...
    CodeView: AptosModuleStorage,
{
    pub(crate) fn execute_entry_function(
        ...
        let function = self.session.code_view.load_function(
            entry_func.module(),
            entry_func.function(),
            entry_func.ty_args(),
        )?;
    ...
    fn check_gas_for_state_changes(
        ...
        // TODO: does this work with profiler / calibration?
        ...
        self.charge_gas_for_state_changes(&mut simulation_gas_meter)
            .map_err(|err| {
                // TODO: remap error to some validation error code?
                err
            })
    }
    ...
    fn charge_gas_for_state_changes(
        ...
        let mut size_tracker = ChangeSetSizeTracker::new(change_set_configs);
        ...
        // if !is_storage_deletion_refund_enabled, do not record the refund.
    ...
    fn is_account_creation_required(&self) -> VMResult<bool> {
            ...
            && self.txn_metadata.replay_protector == ReplayProtector::SequenceNumber(0)
        {
            ...
            return self
                .session
                .data_view
                // TODO: Refactor data view so that the cast is not needed.
                .as_executor_view()
