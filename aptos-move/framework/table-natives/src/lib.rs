...
struct Table {
    ...
    content: BTreeMap<Vec<u8>, VersionedSlot<GlobalValue, MaterializedValue>>,
}
...
impl<'a> NativeTableContext<'a> {
    ...
    pub fn materialize(
        ...
        for (handle, table) in table_data.tables.iter_mut() {
            for (key, slot) in table.content.iter_mut() {
                let state_key = StateKey::table_item(&AptosTableHandle(handle.0), key);

                slot.materialize(current_version, |gv| {
                        ...
                        None => {
                                ...
                                table.value_layout_info.layout_if_contains_delayed_fields()
                            {
                                    ...
                                    .get_read_needing_exchange(&state_key, delayed_field_ids)?
                                {
                                    ...
                                    return Ok(Some(MaterializedValue::Write(
                                        AbstractResourceWriteOp::InPlaceDelayedFieldChange(change),
                                    )));
                                }
                            }

                            return Ok(Some(MaterializedValue::ReadOnly));
                        },
                    };

                    let op = op.and_then(|value| {
                        serialize_value(function_extension, &table.value_layout_info, value)
                    })?;

                    // TODO: cache metadata on read
                    ...
                    Ok(Some(MaterializedValue::Write(
    ...
    pub fn charge_write_ops(
        ...
        let current_version = table_data.vc.current_version();
        for (handle, key, slot) in table_data.tables.iter_mut().flat_map(|(handle, table)| {
            table
                .content
                .iter_mut()
                .map(move |(key, slot)| (handle, key, slot))
        }) {
                ...
                slot.latest_materialization(current_version)
                    .ok_or_else(|| {
                        PartialVMError::new_invariant_violation(
                            "Table items must have been materialized",
                        )
                        .finish(Location::Undefined)
                    })?;
            let write_op = match materialized_value {
                MaterializedValue::ReadOnly => {
                    continue;
                },
                MaterializedValue::Write(write_op) => write_op,
            };
    ...
    pub fn take_writes(&mut self) -> VMResult<BTreeMap<StateKey, AbstractResourceWriteOp>> {
        ...
        for (handle, key, slot) in table_data.tables.iter_mut().flat_map(|(handle, table)| {
            ...
            // TODO: cache state keys.
            let state_key = StateKey::table_item(&AptosTableHandle(handle.0), key);
            let materialized_value = slot
                .take_latest_materialization(current_version)
                .ok_or_else(|| {
                    PartialVMError::new_invariant_violation(
                        "Table items must have been materialized",
                    )
    ...
    pub fn into_change_set(
        ...
        for (handle, table) in tables {
            ...
            for (key, mut slot) in content {
                let op = match slot.take_latest(current_version).into_effect() {
                    Some(op) => op,
                    None => continue,
                };
...
impl LayoutInfo {
    ...
    fn layout_if_contains_delayed_fields(&self) -> Option<Arc<MoveTypeLayout>> {
        self.has_identifier_mappings.then(|| self.layout.clone())
    }
}
    ...
    fn get_or_create_global_value(
        ...
        Ok(match self.content.entry(key) {
            Entry::Vacant(entry) => {
                ...
                let gv = entry
                    .insert(VersionedSlot::new(gv, current_version))
                    .latest_mut(current_version)?
                    .expect("TODO: fixme");
                (gv, Some(loaded))
            },
            Entry::Occupied(entry) => {
                let gv = match entry.into_mut().latest_mut(current_version)? {
                    Some(gv) => gv,
                    None => {
