...
/// Change corresponding to the current aggregator slot: either a write, or a delta that is yet to
/// be applied to the state.
pub enum MaterializedAggregatorWrite {
    Write(WriteOp),
    Delta(DeltaOp),
}
...
pub struct AggregatorData {
    ...
    aggregators:
        BTreeMap<AggregatorID, VersionedSlot<Option<Aggregator>, MaterializedAggregatorWrite>>,
...
impl AggregatorData {
    ...
    pub fn get_aggregator(
        ...
        Ok(match self.aggregators.entry(id) {
            ...
            Entry::Occupied(entry) => {
                ...
                match slot.latest_mut(current_version)? {
                    Some(Some(aggregator)) => return Ok(aggregator),
                    // Should not happen: the native code consumes aggregator when deleted, so it
                    // is not possible to query it again.
                    Some(None) => {
                ...
                slot.set(initialize_aggregator(), current_version)?
                    .as_mut()
                    .expect("Aggregator instance is set when initialized")
            },
        })
    ...
    pub fn create_new_aggregator(
        ...
        match self.aggregators.entry(id.clone()) {
            Entry::Vacant(entry) => {
                entry.insert(VersionedSlot::new(initialize_aggregator(), current_version));
            },
            Entry::Occupied(entry) => {
                let slot = entry.into_mut();
                match slot.latest_mut(current_version)? {
                    ...
                    Some(_) => {
                        let msg = "Aggregator cannot be created twice";
                        return Err(PartialVMError::new_invariant_violation(msg));
                    },
                    // Slot is empty - initialize (below).
                    None => (),
                }
    ...
    pub fn remove_aggregator(&mut self, id: AggregatorID) -> PartialVMResult<()> {
        ...
        match self.aggregators.entry(id) {
            Entry::Vacant(entry) => {
                entry.insert(VersionedSlot::new(None, current_version));
            },
            Entry::Occupied(entry) => {
                let slot = entry.into_mut();
                match slot.latest_mut(current_version)? {
                    Some(aggregator_slot) => {
                        if aggregator_slot.take().is_none() {
                            // Should not happen: the native code consumes aggregator when deleted,
                            // so it is not possible to delete it again.

                            let msg = "Aggregator cannot be deleted twice";
                            return Err(PartialVMError::new_invariant_violation(msg));
                        }
                        return Ok(());
                    },
    ...
    pub fn into(self) -> impl Iterator<Item = (AggregatorID, Option<Aggregator>)> {
        ...
        self.aggregators
            .into_iter()
            .map(move |(id, mut slot)| (id, slot.take_latest(current_version)))
    }
    ...
    pub fn materialize(
        ...
        let current_version = self.version_controller.current_version();
        for (id, v) in self.aggregators.iter_mut() {
            v.materialize(current_version, |slot| -> PartialVMResult<_> {
                // Otherwise, do the materialization. For deletions or writes, we need to fetch state
                // value metadata to attach to write ops.
                let aggregator = match &slot {
                    ...
                    None => {
                        ...
                        return Ok(Some(MaterializedAggregatorWrite::Write(WriteOp::deletion(
                            state_value_metadata,
                        ))));
                    },
                };

                Ok(Some(match &aggregator.state {
                    AggregatorState::Data => {
                        let bytes = serialize(&aggregator.value).into();
                        let write_op = match aggregator_view
                            .get_aggregator_v1_state_value_metadata(&id.0)?
                        {
                            Some(metadata) => WriteOp::modification(bytes, metadata),
                            None => {
                                // Note: aggregator writes historically did not distinguish between
                                // creation and modification.
                                match new_slot_metadata {
                                    None => WriteOp::legacy_modification(bytes),
                                    Some(metadata) => WriteOp::creation(bytes, metadata.clone()),
                                }
                            },
                        };
                        MaterializedAggregatorWrite::Write(write_op)
                    },
                    AggregatorState::PositiveDelta => {
                        let history = aggregator.history.unwrap();
                        let plus = SignedU128::Positive(aggregator.value);
                        let delta_op = DeltaOp::new(plus, aggregator.max_value, history);
                        MaterializedAggregatorWrite::Delta(delta_op)
                    },
                    AggregatorState::NegativeDelta => {
                        let history = aggregator.history.unwrap();
                        let minus = SignedU128::Negative(aggregator.value);
                        let delta_op = DeltaOp::new(minus, aggregator.max_value, history);
                        MaterializedAggregatorWrite::Delta(delta_op)
                    },
                }))
            })?;
    ...
    pub fn write_ops_iter(&mut self) -> impl Iterator<Item = VMResult<(&StateKey, &WriteOp)>> {
        let current_version = self.version_controller.current_version();
        self.aggregators.iter_mut().filter_map(move |(id, slot)| {
            slot.latest_materialization(current_version)
                .map(|write| {
                    use MaterializedAggregatorWrite::*;
                    match write {
                        Write(write_op) => Some((&id.0, write_op)),
                        Delta(_) => None,
                    }
                })
                .ok_or_else(|| {
                    PartialVMError::new_invariant_violation("All writes must be materialized")
    ...
    /// Takes materialized writes for the current version: [WriteOp]s and [DeltaOp]s. Should only
    /// be called after materialization. Returns an error when a non-materialized value is found.
    pub fn take_materialized_writes<'a>(
        &'a mut self,
    ) -> impl Iterator<Item = VMResult<(StateKey, MaterializedAggregatorWrite)>> + 'a {
        let current_version = self.version_controller.current_version();
        self.aggregators.iter_mut().map(move |(id, slot)| {
            slot.take_latest_materialization(current_version)
                .map(|write| (id.0.clone(), write))
                .ok_or_else(|| {
                    PartialVMError::new_invariant_violation("All writes must be materialized")
                        .finish(Location::Undefined)
                })
