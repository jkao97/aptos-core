...
impl DelayedFieldData {
    ...
    pub fn try_add_or_check_delta(
        ...
        match self.delayed_fields.entry(id) {
            ...
            Entry::Occupied(mut entry) => {
                ...
                let change = match entry.get_mut().latest_mut(current_version)? {
                    Some(change) => change,
    ...
    pub fn materialize(&mut self) -> HashSet<DelayedFieldID> {
        ...
        for (id, _) in self.delayed_fields.iter() {
            // TODO:
            //   Figure out what happens if:
            //     1. We add a change here.
            //     2. Change is undone (slot is empty???)
            //   Seems like we cannot just take ids as is, we need to sync for read first and take
            //   ones for non-empty slots.
            ids.insert(*id);
        }
        ids
    }
