...
pub struct TxnLastInputOutput<T: Transaction, O: TransactionOutput<Txn = T>, E: Debug> {
    ...
    committed_fee_statements: Vec<CachePadded<ExplicitSyncWrapper<Option<FeeStatement>>>>,
}
    ...
    TxnLastInputOutput<T, O, E>
{
    ...
    pub fn new(num_txns: TxnIndex) -> Self {
        Self {
            ...
            arced_resource_writes: (0..num_txns)
                ...
                .collect(),
            committed_fee_statements: (0..num_txns)
                .map(|_| CachePadded::new(ExplicitSyncWrapper::<Option<FeeStatement>>::new(None)))
                .collect(),
        }
    }
    ...
    pub(crate) fn commit<S: TStateView<Key = T::Key>>(
        ...
        if let Some((fee_statement, recorded_output_size)) = maybe_fee_statement_and_output_size {
            ...
            if kept_success {
                ...
                *self.committed_fee_statements[txn_idx as usize].dereference_mut() =
                    Some(fee_statement);
            }
    ...
    pub(crate) fn fee_statement(&self, txn_idx: TxnIndex) -> Option<FeeStatement> {
        *self.committed_fee_statements[txn_idx as usize].dereference()
    }
